...
earthTrekSatellite.getSamples = function (tleLine1, tleLine2, startTime, duration, frequency) {
    ...
    positions.addSamples(previousPositions.times, previousPositions.positions);

    var heights = new SampledProperty(Number);
    heights.addSamples(previousPositions.times, previousPositions.heights);

    var velocities = new SampledPositionProperty();
    velocities.addSamples(previousPositions.times, previousPositions.velocities);

    var currentDate = new Date(JulianDate.toIso8601(startTime));
    var latestPositions = this.calculatePositions(tleLine1, tleLine2, startTime, duration, frequency, currentDate);
    positions.addSamples(latestPositions.times, latestPositions.positions);
    heights.addSamples(latestPositions.times, latestPositions.heights);

    velocities.addSamples(latestPositions.times, latestPositions.velocities);
